...
BEGIN
{
  ...
  $postmaster        = 'webmaster@openwaygroup.com';
  @referers          = qw(openwaygroup.com id69.securedata.net);
  @allow_mail_to     = qw(webmaster@openwaygroup.com openwaygroup.com);
  @recipients        = ();
  %recipient_alias   = ();
...
BEGIN {
  $script = __PACKAGE__->new(
     DEBUGGING              => $DEBUGGING,
     name_and_version       => 'NMS FormMail 3.05c1',
